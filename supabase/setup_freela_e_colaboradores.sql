-- =====================================================================
-- setup_freela_e_colaboradores.sql
-- Tudo necessário para o modelo Freela + inserção dos 31 colaboradores
-- Execute no Supabase SQL Editor (uma vez só)
-- =====================================================================

-- ─── 1. Colunas de estrutura ─────────────────────────────────────────

-- Tipo de colaborador (mantido como 'Regular' para todos; coluna legada)
ALTER TABLE employees
  ADD COLUMN IF NOT EXISTS employee_type TEXT NOT NULL DEFAULT 'Regular'
    CHECK (employee_type IN ('Regular', 'Volante'));

-- Favoritar colaboradores (⭐ freelas frequentes)
ALTER TABLE employees
  ADD COLUMN IF NOT EXISTS is_favorite BOOLEAN NOT NULL DEFAULT FALSE;

-- Diária no vínculo (freela fixo usa daily_rate × dias)
ALTER TABLE employee_client_links
  ADD COLUMN IF NOT EXISTS daily_rate NUMERIC(10,2);

-- Tipo de cobertura do freela (Fixo ou Consultoria)
ALTER TABLE employee_client_links
...
;
-- Total: 31 colaboradores | Favoritos (⭐): Anne Ericka, Janaina, Joana D'Arc, Luciene, Silvia

-- ─── 3. Copiar telefone → WhatsApp (para quem já está no banco) ──────

UPDATE employees SET whatsapp = phone WHERE phone IS NOT NULL AND whatsapp IS NULL;
